...
<div id="id04" class="modal">
  <form class="modal-content animate" action="pesan.php" method="post" style="width: 100%;">
    ...
    <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1" style="background-color: black">
     ...
     <?php require('db.php'); $query = "SELECT * FROM users WHERE email='$_SESSION[email]'"; ?>
        <?php if ($query = $con->query($query)): ?>
          <?php while ($data = $query->fetch_array()): ?>
                <li style="margin-top: 0px;"><a href="profile.php?username=<?php echo $data['username'];?>" style="text-decoration: none;"><img src="http://ssl.gstatic.com/accounts/ui/avatar_2x.png" style="width: 30px;height: 30px;margin-right: 5px;border-radius: 20px;"><?php echo $data['username'];?></a></li>
            <?php endwhile ?>
        <?php endif ?>
    <?php require('db.php'); $query = "SELECT * FROM users WHERE email='$_SESSION[email]'"; ?>
        <?php if ($query = $con->query($query)): ?>
          <?php while ($data = $query->fetch_array()): ?>
		<li><a href="beranda1.php?username=<?php echo $data['username'];?>">Home</a></li>
      <?php endwhile ?>
        <?php endif ?>
